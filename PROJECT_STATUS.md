...
- [x] **Detection engine rebuild (2026-06-28)** — a precision-first overhaul targeting the measured failure: the Apr-30 eval showed Sonnet at **46.7% precision, 105 false positives, and 0% Tier-5 recall**. Root causes and fixes:
  ...
  - **Eval extended** with six adversarial false-positive cases (the US Constitution, a non-native-English passage, genuine em-dash/tricolon/from-X-to-Y, a correctly-formal abstract, a short note). Deterministic validation (`eval/validate-rebuild.ts`, no API) confirms: Constitution + clean human prose pass Clean, AI slop scores 118 with burstiness caught, short text gates. The live model precision/recall delta is measured by `npm run eval -- --models sonnet` (needs `ANTHROPIC_API_KEY`); not run in this session.
